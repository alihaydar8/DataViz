function createBubbleChart(error, pokemon) {
  var totals = pokemon.map(function(pokemon) { return +pokemon.total; });
  var meantotal = d3.mean(totals),
      totalExtent = d3.extent(totals),
      totalscaleX,
      totalscaleY;

  var types = d3.set(pokemon.map(function(pokemon) { return pokemon.type1; }));
  var typeColorScale = d3.scaleOrdinal(d3.schemeCategory20)
        .domain(types.values());
    ...
    function createContinentKey() {
      ...
      d3.selectAll("g.continent-key-element")
        .append("rect")
          ...
          .attr("fill", function(d) { return typeColorScale(d); });
  ...
  function createCircles() {
    ...
    circles = svg.selectAll("circle")
      ...
      .enter()
        .append("circle")
        .attr("r", function(d) { return circleRadiusScale(d.total); })
    ...
    function updatePokemonInfo(pokemon) {
      var info = "";
      if (pokemon) {
        info = pokemon.name+'('+ pokemon.type1+') : ' +formatTotal(pokemon.total);
      }
    ...
    function createContinentForces() {
      ...
      function continentForceX(d) {
        if (d.type1 === "Fire" ||  d.type1 === "Water" || d.type1 === "Bug") {
          return left(width);
        } else if (d.type1 === "Ground" || d.type1 === "Grass" || d.type1 === "Normal" ) {
          return right(width);
        }else if (d.type1 === "Rock" || d.type1 === "Electric" || d.type1 === "Poison" ) {
          return leftt(width);
        } else if (d.type1 === "Ice" || d.type1 === "Steel" ||d.type1 === "Psychic") {
          return rightt(width);
        }
        return center(width);
      }
      ...
      function continentForceY(d) {
        if (d.type1 === "Fire" || d.type1 === "Poison" || d.type1 === "Ground" || d.type1 === "Steel" || d.type1 === "Ghost" ) {
          return top(height);
        } else if (d.type1 === "Water" || d.type1 === "Electric" || d.type1 === "Grass" ||d.type1 === "Psychic" ||d.type1 === "Dragon") {
      ...
      function leftt(dimension) { return dimension / 3; }
      function right(dimension) { return dimension / 8 * 7; }
      function rightt(dimension) { return dimension / 11 * 7; }
      function left(dimension) { return dimension / 8; }
      function top(dimension) { return dimension / 8; }
      function bottom(dimension) { return dimension / 8 * 7; }
    }
  ...
  function addGroupingListeners() {
    ...
    function toggleTotalAxes(showAxes) {
      ...
      function createAxes() {
        ...
        var xAxis = d3.axisBottom(totalScaleX)
        ...
        var yAxis = d3.axisLeft(totalScaleY)
          .ticks(numberOfTicks, tickFormat);
        svg.append("g")
          .attr("class", "y-axis")
